...
void GenerateKeystream_ZUC(u32* pKeystream, int KeystreamLen)
{
	int i;
	{
		BitReorganization();
...
void ZUC_EEA3( unsigned char *key, unsigned int COUNT, unsigned int BEARER, unsigned int DIRECTION,unsigned char *data,  unsigned int length )
{
	...
	u32* C=(unsigned int *)malloc(length+1);
	
	EEA3(key,COUNT,BEARER,DIRECTION,length*8, M,C);
	//EEA31(key,IV,count,bearer,dir,length, M,C);
	//const char* out=(char *)malloc(datalen+1);
	
	memcpy(data,C,length);
